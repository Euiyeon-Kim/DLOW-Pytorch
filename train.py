...
if __name__ == "__main__":
    ...
    logger = Logger(train_conf, num_batches)
    ...
    for epoch in range(train_conf['start_epoch'], train_conf['start_epoch'] + train_conf['num_epochs']):
        ...
        for i, batch in enumerate(train_dl):
            ...
            # Logging on terminal
            sys.stdout.write('\nEpoch %03d/%03d [%04d/%04d] -- '%(epoch, train_conf['num_epochs'], i+1, num_batches))
            sys.stdout.flush()

            # Logging on tensorboard
            if i % train_conf['save_summary_steps'] == 0:
                loss_log, img_log = model.get_data_for_logging()
                logger.log(loss_log, img_log)
        
        if (epoch % 10) == 0: # validation 성능으로 수정하기

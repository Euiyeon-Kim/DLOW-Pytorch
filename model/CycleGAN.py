...
class Generator(nn.Module):
    def __init__(self, input_nc, output_nc, output_filter_num=64, use_dropout=False, n_residual_blocks=9):
        ...
        model = [   nn.ReflectionPad2d(3),
                    ...
                    nn.ReLU(inplace=True)   ] # inplace : input자체를 변경 --> 메모리 절약 가능
              
        n_downsampling = 2
        for i in range(n_downsampling):  # Downsampling
            mult = 2 ** i
            model += [nn.Conv2d(output_filter_num * mult, output_filter_num * mult * 2, kernel_size=3, stride=2, padding=1),
        ...
        for i in range(n_downsampling):  # Upsampling
            ...
            model += [  nn.ConvTranspose2d(output_filter_num * mult, int(output_filter_num * mult / 2),
